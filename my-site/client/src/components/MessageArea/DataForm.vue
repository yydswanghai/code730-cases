<template>
  <form class="data-form-container" ref="form" id="data-form-container" @submit.prevent="handleSubmit">
      <div class="form-item">
...
export default {
    ...
    methods: {
        handleSubmit(){
            ...
            this.$emit("submit", this.formData, (successMsg) => {// 传递回调，等父级处理完，还有其他事情
...
button{
    ...
    background: @primary;
    &:hover{
        background: darken(@primary, 10%);// 降低
    }
    &:disabled{
        background: lighten(@primary, 20%);// 变亮
        cursor: not-allowed;
    }
}
</style>